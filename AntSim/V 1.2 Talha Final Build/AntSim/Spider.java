    ...
    public void act() {
        if (state == AntState.IDLE){
            ...
            if(energy < energyThreshold){
                List<DeadAnt> prospects = getObjectsInRange(sightRange,DeadAnt.class);
                //if there is Ant
                if (prospects.size() > 0){
                    //set to foraging
            ...
            else if (energy < energyThreshold && getObjectsInRange(sightRange, Ant.class).size()>0){
                state = AntState.IDLE;
            }
            ...
            spoodermandusted = new DeadSpider();
            getWorld().addObject(spoodermandusted, getX(), getY());
